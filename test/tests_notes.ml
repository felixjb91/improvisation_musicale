...
let test_note_factory_1 () =
  Alcotest.(check note_name) "same nom"  Do note_1.nom
  Alcotest.(check alteration) "same alter"  Diese note_1.alter
  Alcotest.(check int) "same octave" 3 note_1.octave

let note_2 = note_factory Mi Diese 1

let test_note_factory_2 () =
  Alcotest.(check note_name) "same nom" Do note_2.nom
  Alcotest.(check alteration) "same alter" Becarre note_2.alter
...
let test_note_factory_type_1 () =
  Alcotest.(check typed_note) "same type_de_note"  Noire typed_note.type_de_note
...
let numNote2nom_1 () =
  Alcotest.(check note_name) "same nom"  Fa numNote2nom
...
let note2midi_1 () =
  Alcotest.(check int) "same nom" 1 note2midi
